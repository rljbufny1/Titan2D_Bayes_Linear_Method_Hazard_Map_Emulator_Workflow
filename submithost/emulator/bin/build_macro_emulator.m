...
function build_macro_emulator(datadir_)

    datadir = strcat(datadir_, '/');
        
    ifplot=0; %1;
    fid=fopen(strcat(datadir, 'uncertain_input_list.txt'),'r');
    if(fid==-1)
    ...
    fid=fopen(strcat(datadir, 'macro_emulator.pwem'),'w');
    ...
    %then row sorting putting lowest node numbers first (lowest node number
