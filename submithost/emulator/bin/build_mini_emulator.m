%**********************************************************************
% matlab script: build_mini_emulator.m
%
% argument(s): datadir_,samplenumber
%
% input(s): build_mini_pwem_meta.<samplenumber - formatted as %06g>,
%   down_sampled_data.<samplenumber - formatted as %06g> 
%   (needs all the down_sampled_data files)
%
% output(s): mini_emulator.<samplenumber - formatted as %06g>
%**********************************************************************

function build_mini_emulator(datadir_,samplenumber)
    ...
    filename = strcat(datadir, sprintf('build_mini_pwem_meta.%06g',samplenumber));
    fid=fopen(filename,'r');
    if (fid==-1)
       disp(filename); disp(' not found');
       return;
    end
    ...
    for ineighmacro=1:Nneighmacro
        ...
        filename=strcat(datadir, sprintf('down_sampled_data.%06g',jyneighmacro));
        fid=fopen(filename,'r');
        if(fid==-1)
            disp(filename); disp(' not found');
            return;
        ...
        if(ineighmacro==1)
            ...
            % rlj
            % reference:
            % https://groups.google.com/forum/#!topic/mainsar/rAA7yc3CEXc:
            %dsearch(x,y,TRI,xi,yi) returns the index of the nearest (x,y) point to the point (xi,yi). 
            %dsearch requires a triangulation TRI of the points x,y obtained from delaunay.
            %Z=dsearch(x,y,ele(:,2:4),X,Y); %index from grid to pixel node
            % to
            %Z=dsearchn([x,y],ele(:,2:4),[X(:),Y(:)]); 
            
            %Iynearestnew=dsearch(ysynew(:,1),ysynew(:,2),tessnew,ysy(Iy0N,1),ysy(Iy0N,2));
            Iynearestnew=dsearchn([ysynew(:,1),ysynew(:,2)],tessnew,[ysy(Iy0N,1),ysy(Iy0N,2)]);
            IynewN=1:NynewN;
            for iy0=1:Ny0N
    ...
    if(ifwrite)
        ...
        filename = strcat(datadir,sprintf('mini_emulator.%06d',samplenumber));
    ...
    %for iy0=399:401;
        ...
        for itheta=1:NThetaGuesses
            ...
            [crit,sigma,theta,r]=GuessGoodThetaSigma(samplenumber,4,Nfree,epsn,...
...
function [CRIT,SIGMA,THETA,R]=...
    GuessGoodThetaSigma(samplenumber,Nnewt,Nfree,eps,...
    ...
    [THETA,R,rcondR,eigvect,eigval,d1rcondRdti,Q1,Q2]=...
        increase_rcondR_to_minrcondR(samplenumber,Z,minrcondR,THETA);
...
function [THETA,R,rcondR,eigvect,eigval,d1rcondRdti,Q1,Q2]=...
    ...
    for iter=1:Niter+1
        ...
        % There are now 16 different code paths underlying the eig function, 
        % depending on whether there are one or two arguments, 
        % whether the arguments are real or complex, 
        % whether the problem is symmetric and whether the eigenvectors are requested
        ...
        try
           ...
           fprintf('build_mini_emulator increase_rcondR_to_minrcondR eig error 1 sample number: %d, iter: %d\n',...
              samplenumber, iter);
          
           try
               
              % rlj per Dr. Abani Patra
              %Mod to prevent EIG does not converge error
              IR = eye(size(R));
              [eigvect,eigval] = eig(R,IR);
           
           catch
               
              fprintf('build_mini_emulator increase_rcondR_to_minrcondR eig error 2 sample number: %d, iter: %d\n',...
                 samplenumber, iter);
             
              % Save the current workspace to a .mat file
              filename = sprintf('DEBUG_EIG_ERROR_SAMPLE_NUMBER_%d', samplenumber)
              save (filename);
        ...
        %0/0; ? For octave 4.0.0 causing multiple division by zero errors
