...
function identify_mini_emulators_to_evaluate(datadir_)
    ...
    if(NdiminmacroY~=NdiminmacroX)
        ...
        disp('NdiminmacroY not equal to NdiminmacroX');
        return;
    end
...
if(0)
    ...
    % rlj - Handle when isnan returns an array of all 0s
    inan=find(isnan(isimp),1):length(isimp);
    %inotnan=1:inan(1)-1;
    if (isempty(inan))
       inotnan=1:length(isimp);
    else
       inotnan=1:inan(1)-1;
    end
    isimp=isimp(inotnan);
