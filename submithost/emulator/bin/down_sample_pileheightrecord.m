%**********************************************************************
% octave script: down_sample_pileheightrecord.m
%
% argument(s): datadir_,samplenumber
%
% input(s): uncertain_input_list.txt, 
%   pileheightrecord.<sample number - formatted as %05d>
%
% output(s): down_sampled_data.<samplenumber - formatted as %06g>
%**********************************************************************

function down_sample_pileheightrecord(datadir_,samplenumber)
    ...
    if(fid==-1)
        disp('uncertain_input_list.txt not found');
        return;
    end
    ...
    % TODO: check for geoflow tiny in simulation.py?
    GEOFLOW_TINY=volume^(1/3)/10000;

    filename=strcat(datadir, sprintf('pileheightrecord.%06g',samplenumber));
fid=fopen(filename,'r');
    if(fid==-1)
        fprintf('error opening the pile height record %s for sample %d\n',...
...
%iyesh=(iy_yesh-1)*Nx+ix_yesh;
    ...
    while(Nmore<Nwant)
        %TOOSMALL
        % rlj - delaunay function no longer available
        %trit=delaunay(xsofar,ysofar,{'QJ'})';
        trit=delaunayn([xsofar,ysofar],{'QJ'})';
    ...
    if(ifwrite)        
        ...
        write_down_sampled_data(filename,log10vol,UTMEastStart,UTMNorthStart,BedFrictAng,x,y,h,minmaxh);
    end
...
%write the DOWN SAMPLED DATA FILE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function write_down_sampled_data(filename,log10vol,UTMEastStart,UTMNorthStart,BedFrictAng,X,Y,H,minmaxh)
    iftoc=0;
    ...
    fprintf(fid,'%g\n%.10g %.10g %.10g %.10g\n%d %d\n',4,log10vol,UTMEastStart,UTMNorthStart,BedFrictAng,Nx,Ny);
    for iy=1:Ny
