%**********************************************************************
% matlab script: evaluate_mini_emulator_mean.m
%
% argument(s): datadir_,samplenumber, macrosimplex_resample_filename
%
% input(s): mini_emulator.*(needs all the mini_emulator files)
%
% output(s): mini_emulator_eval_<simplex id string>
%**********************************************************************

function evaluate_mini_emulator_mean(datadir_,samplenumber,macrosimplex_resample_filename)
    ...
    miniemulatorfilename=strcat(datadir, sprintf('mini_emulator.%06d',samplenumber));
...
%         fprintf(fid,'%%{{iyneigh: index of y neighbor: [1] integer},{ihop: #of (y0,ymacro) hops from center of micro-emulator: [3] integers},{Rinveps: R^-1*eps (eps = sy minus unadjusted micro-emulator mean): [1] double}}\n');
    ...
    pwem=repmat(struct('iyneigh',[],'thetaTRAN',[],'beta',[],'Rinveps',[]),Ny0N,1);
     ...
     for iy0=1:Ny0N
        if feof(fid) 
            fprintf('ERROR: %s end of file encountered. Ny0N = %d, iy0 = %d!!!!\n',...
               miniemulatorfilename, Ny0N, iy0);   
            bob; %cause the code to crash
        else
        %blah=fgets(fid);
...
%         fprintf(fid,'%%     [Nxspatial] lines containing {{(east,north): [2] doubles},{[Nmacroseachx] sets of {(log10(volume [m^3]),Direction [deg CC from east],BedFrictAng [deg],IntFrictAng [deg]): macro input coordinates: [Ndiminmacro] doubles}}}\n');
    ...
    switch(fileformatoption)
        case 1,
            ...
            % rlj - tsearch no longer exists
            %T=repmat(tsearch(ysy(Iy0T,1),ysy(Iy0T,2),tess0,xspatial(:,1),xspatial(:,2)),Nmacroseachx,1); 
            % Split up to see what is going on
            arg1 = [ysy(Iy0T,1),ysy(Iy0T,2)];
            arg2 = [xspatial(:,1),xspatial(:,2)];
            arg3 = tsearchn(arg1,tess0,arg2);
            T=repmat(arg3,Nmacroseachx,1);        
        ...
        case 2,            
            ...
            if(Nmacroseachx==1)
                ...
                T=tsearchn([ysy(Iy0T,1),ysy(Iy0T,2)],tess0,[xsx(:,1),xsx(:,2)]);
            else
                xspatial=yada(:,1:Ndiminspatial);
                %T=repmat(tsearch(ysy(Iy0T,1),ysy(Iy0T,2),tess0,xspatial(:,1),xspatial(:,2)),Nmacroseachx,1);
                T=repmat(tsearchn([ysy(Iy0T,1),ysy(Iy0T,2)],tess0,[xspatial(:,1),xspatial(:,2)]),Nmacroseachx,1);
...
%                with tsearchn
    ...
    filenameout=strcat(datadir, sprintf('mini_emulator_eval_%s',UniqueMiniEmulatorKey));
