%**********************************************************************
% matlab script: merge_probability_of_hazard_maps.m
%
% argument(s): AZ_vol_dir_bed_int_final.phm, phm_from_eval* filename
%
% input(s): AZ_vol_dir_bed_int_final.phm, phm_from_eval* file
%
% returns: errorcode
%
...
%"wrong filenames" do not result in a saved DEBUG*.mat file.
    ...
    scanstr = strcat(datadir, 'phm_from_eval_%d.%d.%d');
    yada=sscanf(temporaryhazmapfilename,scanstr,3);
    if(numel(yada)~=3)
...
%     fprintf(fid,'%%[Nresamp] lines containing the {{resample macro-inputs: (log10(volume [m^3]),Direction [deg CC from east],BedFrictAng [deg],IntFrictAng [deg]): [Ndiminmacro] doubles},{w: weight of this resample macro-input: [1] double}}\n');
    ...
    %fprintf('Moving %s to %s\n', newpermanenthazmapfilename, permanenthazmapfilename)
    movefile (newpermanenthazmapfilename, permanenthazmapfilename);
